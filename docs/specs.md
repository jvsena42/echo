...
- **`.apkg` import (after that)** — a zip containing a SQLite collection (`collection.anki2`, or `collection.anki21b` = zstd-compressed on 2.1.50+), a media manifest, and numbered blobs. Needs a KMP zip reader, a SQLite driver, and zstd — three new dependencies for content already reachable as tab-separated text, so it waits until the `.txt` path has proven the scale.
