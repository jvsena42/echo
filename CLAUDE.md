...
**Still Android-only:** Speak (needs a `SpeechRecognizer` binding plus `Info.plist` usage strings), bulk import and triage, tag browse, and — per #149 — the signup, backup and restore screens. `IosMediaProcessor` and `IosSpeechRecognizer` do not exist, so **do not add an `IosDependencies` accessor for `BulkImportViewModel`**: it takes a `MediaProcessor` and would throw at resolution. iPad and any width but a phone is #173.
