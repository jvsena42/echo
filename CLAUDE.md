...
- **A deck is announced once, and the ledger — not the deck id — is what says so.** Every announce mints a fresh post id, so nothing stopped one deck being posted about twice; a deck deleted and re-imported did exactly that, leaving an older post pointing at a manifest that no longer resolves (#145). `DiscoveryRepository.announcedPost` answers from `/pub/loopky/announcements/{sha256}.json`, keyed by `DeckAnnouncement.dedupeKey` — kind + normalized title + source author — **deliberately not the deck URI**, because a re-published deck has a new `deckId` and that is the case being caught. The check sits at the write as well as at the three prompts, for the same reason the `shareOnPubky` gate does. Two things not to undo: an unreadable ledger answers "not announced" (asking twice is recoverable, silently dropping a real deck's announcement is not), and the ledger entry is written **after** the post. Deleting a deck leaves both its post and its entry alone. See Architecture.md §8.0.
